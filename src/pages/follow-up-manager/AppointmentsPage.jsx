...
import { Field, inputCls, getApiErrorMessage, ListingTypeFilter, ListingBadge } from '../../components/leads/leadShared'
...
function LeadPickerPanel({ onClose, onPick }) {
  ...
  return (
    <SidePanel onClose={onClose} icon={Calendar} iconColor="#8B5CF6" title="New Appointment" subtitle="Which hot lead is this for?">
      <div className="px-5 py-4">
        {isLoading ? (
          ...
          <p className="text-sm text-[#6B7280] dark:text-[#A1A1AA] text-center py-10">
            No hot leads in your queue yet.
          </p>
        ) : (
          <ul className="space-y-1.5">
            {leads.map((l) => (
              <li key={l._id}>
                <button
                  onClick={() => onPick(l)}
                  className="w-full flex items-center gap-3 px-3.5 py-3 rounded-xl border border-[#E5E7EB] dark:border-[#2A2A2A] hover:border-[#8B5CF6]/40 hover:bg-[#8B5CF6]/5 text-left transition-all"
                >
                  <div className="flex-shrink-0 w-8 h-8 rounded-lg bg-[#8B5CF6]/10 flex items-center justify-center">
                    <MapPin className="w-4 h-4 text-[#8B5CF6]" strokeWidth={1.75} />
                  </div>
...
/* ─── Reschedule Panel (managing the lead, not the assignee) ────────────── */
...
function ReschedulePanel({ appt, onClose, onSaved }) {
  ...
  const mut = useMutation({
    ...
    onError: (err) => toast.error(getApiErrorMessage(err, 'Failed to reschedule')),
  })
      ...
      footer={
        <>
          ...
          <button
            ...
            className="flex-1 py-3 rounded-xl text-sm font-semibold text-white bg-[#8B5CF6] hover:bg-[#7C3AED] disabled:opacity-60 flex items-center justify-center gap-2"
          >
      ...
      <div className="flex items-center gap-2 mt-3 pt-3 border-t border-[#F5F5F4] dark:border-[#202020]">
        ...
        {!isAssignee && ['scheduled', 'confirmed'].includes(appt.status) && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-[#8B5CF6]/10 text-[#8B5CF6] hover:bg-[#8B5CF6]/20"
          >
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center px-4">
      <div className="w-16 h-16 rounded-2xl bg-[#8B5CF6]/10 flex items-center justify-center mb-4">
        <Calendar className="w-8 h-8 text-[#8B5CF6]" strokeWidth={1.5} />
      </div>
      ...
      <p className="text-sm text-[#6B7280] dark:text-[#A1A1AA] mb-6 max-w-xs">
        {hasFilters ? 'Try a different status filter.' : 'Appointments booked for you, or for hot leads you manage, will show up here.'}
      </p>
      {!hasFilters && (
        <button onClick={onAdd} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold text-white bg-[#8B5CF6] hover:bg-[#7C3AED]">
...
export default function FollowUpManagerAppointmentsPage() {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
          <div>
            <h1 className="text-xl font-bold text-[#111111] dark:text-white tracking-tight flex items-center gap-2.5">
              <Calendar className="w-5 h-5 text-[#8B5CF6]" strokeWidth={1.75} />
              Appointments
            </h1>
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white bg-[#8B5CF6] hover:bg-[#7C3AED] shadow-sm hover:shadow-md active:scale-[0.98] transition-all self-start sm:self-auto"
          >
            <Plus className="w-4 h-4" />
            New Appointment
          </button>
