...
const EVENT_META = {
  new_lead:             { label: 'New Lead',        icon: FileText,     color: '#8B5CF6' },
...
function NotificationCard({ n, onMarkRead, isMarking }) {
  ...
  return (
    <div className={`flex items-start gap-3 p-4 rounded-2xl border transition-all ${
      unread
        ? 'bg-[#F5F0FE] dark:bg-[#1A1625] border-[#8B5CF6]/25'
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {unread && <span className="w-1.5 h-1.5 rounded-full bg-[#8B5CF6] flex-shrink-0" />}
        </div>
      ...
      {unread && (
        <button
          ...
          disabled={isMarking}
          title="Mark as read"
          className="flex-shrink-0 flex items-center justify-center w-7 h-7 rounded-lg text-[#6B7280] dark:text-[#A1A1AA] hover:bg-[#8B5CF6]/10 hover:text-[#8B5CF6] disabled:opacity-50 transition-colors"
        >
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center px-4">
      <div className="w-16 h-16 rounded-2xl bg-[#8B5CF6]/10 flex items-center justify-center mb-4">
        <Bell className="w-8 h-8 text-[#8B5CF6]" strokeWidth={1.5} />
      </div>
      ...
      <p className="text-sm text-[#6B7280] dark:text-[#A1A1AA] max-w-xs">
        {unreadOnly
          ...
          : 'Appointment reminders, listing updates and commission alerts will appear here.'}
      </p>
    </div>
  )
}

export default function FollowUpManagerNotificationsPage() {
  const qc = useQueryClient()
  const [unreadOnly, setUnreadOnly] = useState(false)
  const [page, setPage] = useState(1)

  const { data, isLoading, isError } = useQuery({
    queryKey: ['fum-notifications-page', { page, unreadOnly }],
  ...
  const markReadMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['fum-notifications-page'] })
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
          <div>
            <h1 className="text-xl font-bold text-[#111111] dark:text-white tracking-tight flex items-center gap-2.5">
              <Bell className="w-5 h-5 text-[#8B5CF6]" strokeWidth={1.75} />
              Notifications
            </h1>
          ...
          {unreadCount > 0 && (
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white bg-[#8B5CF6] hover:bg-[#7C3AED] disabled:opacity-60 shadow-sm hover:shadow-md active:scale-[0.98] transition-all self-start sm:self-auto"
            >
          ...
          {[{ key: false, label: 'All' }, { key: true, label: 'Unread' }].map((tab) => (
            ...
            >
              ...
              {tab.key && unreadCount > 0 && (
                <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-[#8B5CF6]/15 text-[#8B5CF6]">{unreadCount}</span>
              )}
            </button>
          ))}
        </div>
