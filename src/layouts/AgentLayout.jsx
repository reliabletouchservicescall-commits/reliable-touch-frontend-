...
const NAV = [
  {
    group: 'Overview',
    items: [
      { label: 'Dashboard', icon: LayoutDashboard, to: '/agent', end: true },
    ],
  },
  {
    group: 'My Work',
    items: [
      { label: 'My Leads', icon: TrendingUp, to: '/agent/leads' },
      { label: 'Appointments', icon: CalendarCheck, to: '/agent/appointments' },
    ],
  },
  {
    group: 'Finance',
    items: [
      { label: 'Commissions', icon: DollarSign, to: '/agent/commissions', pending: true },
      { label: 'Lease Renewals', icon: RotateCcw, to: '/agent/lease-renewals', pending: true },
    ],
  },
  {
    group: 'Account',
...
function NavItem({ icon: Icon, label, to, end, pending, collapsed, onClick }) {
  return (
    <NavLink
      to={to}
      end={end}
    ...
    >
      <Icon className={`flex-shrink-0 w-[18px] h-[18px] ${collapsed ? 'mx-auto' : ''}`} strokeWidth={1.75} />
      {!collapsed && (
        <>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5 px-4 py-5 border-b border-[#E5E7EB] dark:border-[#2A2A2A]">
        <div className="flex-shrink-0 flex items-center justify-center w-8 h-8 rounded-lg bg-[#10B981]">
        ...
        {(!collapsed || isMobile) && (
          <div className="min-w-0">
            ...
            <p className="text-[9px] uppercase tracking-widest font-semibold text-[#10B981]">
              Agent
            </p>
          </div>
        )}
      ...
      <div className="px-3 pb-4 pt-3 border-t border-[#E5E7EB] dark:border-[#2A2A2A] space-y-1">
        ...
        {(!collapsed || isMobile) && (
          <div className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-[#F5F5F4] dark:bg-[#202020]">
            <div className="flex-shrink-0 w-7 h-7 rounded-full bg-[#10B981]/15 flex items-center justify-center">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] text-[#6B7280] dark:text-[#A1A1AA] truncate">Agent</p>
            </div>
            <button
